...
from layers.feedforward import conv, normalization
...
def build_model(data_tensor, reuse, training, output_shape):
    ...
    with tf.variable_scope('cnn', reuse=reuse):
        ...
        h2 = normalization.batch(
            bottom=h2, renorm=False, name='hgru_bn', training=training
        )
        activity = conv.readout_layer(
            activity=h2, reuse=reuse, training=training, output_shape=output_shape
        )
