...
def train_status(log, dt, step, train_loss, rate, timer, score_function, train_score):
    """Print training status."""
    format_str = (
    ...
    log.info(
        format_str % (dt, step, train_loss, rate, timer, score_function, train_score)
    )


def training_step(sess, train_dict, feed_dict=False):
...
def validation_step(
    ...
    for num_vals in tqdm(range(config.validation_steps)):
        ...
        it_val_score = np.append(it_val_score, it_val_dict['val_score'])
...
def save_progress(
    ...
    ckpt_path = os.path.join(directories['checkpoints'], 'model_%s.ckpt' % step)
...
def training_loop(
    ...
    if placeholders:
        ...
        train_batch_idx = (
            np.arange(train_batches / config.train_batch_size)
            .reshape(-1, 1)
            .repeat(config.train_batch_size)
        )
        ...
        val_batch_idx = (
            np.arange(val_batches / config.val_batch_size)
            .reshape(-1, 1)
            .repeat(config.val_batch_size)
        )
        val_images = val_images[: len(val_batch_idx)]
        val_labels = val_labels[: len(val_batch_idx)]
        raise NotImplementedError
        for epoch in tqdm(range(config.epochs), desc='Epoch', total=config.epochs):
            for train_batch in range(train_batches):
                ...
                if isinstance(it_train_images[0], basestring):
                    it_train_images = np.asarray(
                        [data_to_tfrecords.load_image(im) for im in it_train_images]
                    )
                feed_dict = {
                    train_dict['train_images']: it_train_images,
                    train_dict['train_labels']: it_train_labels,
                }
                (train_score, train_loss, it_train_dict, timer) = training_step(
                    sess=sess, train_dict=train_dict, feed_dict=feed_dict
                )
        ...
        try:
            while not coord.should_stop():
                (train_score, train_loss, it_train_dict, duration) = training_step(
                    sess=sess, train_dict=train_dict
                )
                if step % config.validation_period == 0:
                    val_score, val_lo, it_val_dict, duration = validation_step(
