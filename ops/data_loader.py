...
def resize_image_label(im, model_input_image_size, f='bilinear'):
    ...
    if len(im.get_shape()) > 3:
        ...
        for idx in range(len(sims)):
            ...
            im = sims[idx]
            sims[idx] = res_fun(im, model_input_image_size, align_corners=True)
        im = tf.squeeze(tf.stack(sims))
        if len(im.get_shape()) < 4:
            im = tf.expand_dims(im, axis=-1)
    else:
        im = res_fun(
            tf.expand_dims(im, axis=0), model_input_image_size, align_corners=True
        )
...
def crop_image_label(image, label, size, crop='random'):
    ...
    if crop == 'random':
        ...
        label = tf.image.resize_image_with_crop_or_pad(label, size[0], size[1])
        return image, label
...
def random_crop(image, model_input_image_size):
    ...
    if len(im_size) == 3:
        ...
        crop_image = tf.expand_dims(
            tf.transpose(tf.random_crop(trans_image, crop_size), [2, 0, 1]), axis=-1
        )
        return crop_image
    else:
        raise NotImplementedError
...
def image_augmentations(image, data_augmentations, model_input_image_size, label=None):
    """Coordinating image augmentations for both image and heatmap."""
    im_size = [int(x) for x in image.get_shape()]
    im_size_check = np.any(np.less_equal(model_input_image_size[:2], im_size[:2]))
    if data_augmentations is not None:
        ...
        if 'rotate' in data_augmentations and im_size_check:
            max_theta = 22.0
            angle_rad = (max_theta / 180.0) * math.pi
        ...
        if 'rotate_image_label' in data_augmentations and im_size_check:
            ...
            print 'Applying random rotate.'
        if 'random_scale_crop_image_label' in data_augmentations and im_size_check:
            scale_choices = tf.convert_to_tensor([1.0, 1.02, 1.04, 1.06, 1.08])
            samples = tf.multinomial(tf.log([tf.ones_like(scale_choices)]), 1)
            image_shape = image.get_shape().as_list()
            scale = scale_choices[tf.cast(samples[0][0], tf.int32)]
            scale_tf = tf.cast(
                tf.round(
                    np.asarray(model_input_image_size[:2]).astype(np.float32) * scale
                ),
                tf.int32,
            )
            combined = tf.concat([image, label], axis=-1)
        ...
        if 'rc_res' in data_augmentations and im_size_check:
            ...
            image = resize_image_label(im=image, model_input_image_size=ms, f='bicubic')
        ...
        if 'random_crop_image_label' in data_augmentations and im_size_check:
            ...
            image, label = crop_image_label(
                image=image, label=label, size=model_input_image_size, crop='random'
            )
        if 'center_crop_image_label' in data_augmentations and im_size_check:
            assert len(image.get_shape()) == 3, '4D not implemented yet.'
            image, label = crop_image_label(
                image=image, label=label, size=model_input_image_size, crop='center'
            )
        if 'resize' in data_augmentations and im_size_check:
            if len(model_input_image_size) > 2:
                model_input_image_size = model_input_image_size[:2]
            image = resize_image_label(
                im=image, model_input_image_size=model_input_image_size, f='bicubic'
            )
        ...
        if 'resize_and_crop' in data_augmentations and im_size_check:
            model_input_image_size_1 = np.asarray(model_input_image_size[:2]) + 28
            image = resize_image_label(
                im=image, model_input_image_size=model_input_image_size_1, f='area'
            )
            image = center_crop(image, model_input_image_size)
            print 'Applying area resize.'
        if 'resize_nn' in data_augmentations and im_size_check:
            ...
            if len(model_input_image_size) > 2:
                model_input_image_size = model_input_image_size[:2]
            image = resize_image_label(
                im=image, model_input_image_size=model_input_image_size, f='nearest'
            )
        ...
        if 'resize_image_label' in data_augmentations and im_size_check:
            ...
            label = resize_image_label(
                im=label, model_input_image_size=model_input_image_size, f='bicubic'
            )
            ...
            label = resize_image_label(
                im=label, model_input_image_size=model_input_image_size, f='nearest'
            )
            print 'Applying nearest resize.'
        else:
            pass
        ...
        if 'calculate_rate_time_crop' in data_augmentations:
            ...
            time_crop = tf.random_uniform(
                [], minval=minval, maxval=im_shape[0], dtype=tf.int32
            )
            ...
            padded_image = tf.zeros(
                [im_shape[0] - time_crop] + im_shape[1:], dtype=selected_image.dtype
            )

            # Randomly concatenate pad to front or back
            image = tf.cond(
                pred=tf.greater(
                    tf.random_uniform([], minval=0, maxval=1, dtype=tf.float32), 0.5
                ),
...
def decode_data(features, reader_settings):
    ...
    if features.dtype == tf.string:
        return tf.decode_raw(features, reader_settings)
    else:
        return tf.cast(features, reader_settings)
...
def read_and_decode(
    ...
    if number_of_files == 1:
        ...
        _, serialized_examples = reader.read_up_to(
            filename_queue, num_records=number_of_files
        )
        features = tf.parse_example(serialized_examples, features=tf_dict)

    # Handle decoding of each element
    image = decode_data(
        features=features['image'], reader_settings=tf_reader_settings['image']['dtype']
    )
    label = decode_data(
        features=features['label'], reader_settings=tf_reader_settings['label']['dtype']
    )
    ...
    if aux is not None:
        ...
        aux = tf.reshape(aux_data, tf_reader_settings[aux.keys()[0]]['reshape'])
    else:
        aux = tf.constant(0)
        ...
        if resize_output is not None:
            ...
            if isinstance(resize_output, dict):
                if resize_output.keys()[0] == 'resize':
                    label = resize_image_label(
                        im=label, model_input_image_size=resize_output, f='nearest'
                    )
            ...
            for im, lab in zip(split_images, split_labels):
                ...
                if resize_output is not None:
                    ...
                    it_lab = resize_image_label(
                        im=it_lab, model_input_image_size=resize_output, f='area'
                    )
                images += [it_im]
                labels += [it_lab]
