...
def import_module(module, pre_path='datasets'):
    ...
    return getattr(__import__(pre_path, fromlist=[module]), module)
...
def prepare_directories(config, exp_label):
    ...
    dir_list = {
        'checkpoints': os.path.join(config.checkpoints, exp_label),
        'summaries': os.path.join(config.summaries, exp_label),
        'condition_evaluations': os.path.join(config.condition_evaluations, exp_label),
        'weights': os.path.join(config.condition_evaluations, exp_label, 'weights'),
    }
...
def save_npys(data, model_name, output_string):
    ...
    for k, v in data.iteritems():
        k = k.replace('/', '_')
        output = os.path.join(output_string, '%s_%s' % (model_name, k))
        try:
            np.save(output, v)
        except Exception:
...
def get_data_pointers(dataset, base_dir, cv):
    ...
    check_path(data_pointer, '%s not found.' % data_pointer)
    mean_loc = check_path(
        data_means, '%s not found for cv: %s. Trying .npz fallback.' % (data_means, cv)
    )
    data_means_image, data_means_label = None, None
    if not mean_loc:
        alt_data_pointer = data_means.replace('.npy', '.npz')
        alt_data_pointer = check_path(
            alt_data_pointer, '%s not found. npz fallback failed.' % alt_data_pointer
        )
        ...
        if not alt_data_pointer:
            ...
            if data_means_image is not None and isinstance(data_means_image, np.object):
                data_means_image = data_means_image.item()
            if data_means_label is not None and isinstance(data_means_label, np.object):
