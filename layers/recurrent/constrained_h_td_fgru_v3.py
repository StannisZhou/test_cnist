...
class hGRU(object):
    ...
    def __init__(
        ...
        if up_kernel is None:
            self.up_kernel = [
                x + y for x, y in zip(self.pool_strides, self.pool_kernel)
            ]
        ...
        assert isinstance(
            self.intermediate_repeats, list
        ...
        self.mu_initializer = tf.initializers.constant(0.0)
    ...
    def prepare_tensors(self):
        ...
        for idx, (ff_filters, ff_kernel, reps) in enumerate(
            zip(self.intermediate_ff, self.intermediate_ks, self.intermediate_repeats)
        ):
            if self.residual:
                if 0:  # elf.batch_norm:
                    setattr(
            ...
            for il in range(reps):
                setattr(
                    ...
                    tf.get_variable(
                        name='%s_ffdrive_kernel_%s_%s' % (self.layer_name, idx, il),
                        dtype=self.dtype,
                ...
                setattr(
                    ...
                    tf.get_variable(
                        name='%s_ffdrive_bias_%s_%s' % (self.layer_name, idx, il),
                        dtype=self.dtype,
                        shape=[ff_filters],
                        initializer=tf.initializers.ones(),
                        trainable=True,
                ...
                if self.batch_norm:
                    setattr(
                        ...
                        tf.get_variable(
                            name='%s_ffdrive_gamma_%s_%s' % (self.layer_name, idx, il),
                            dtype=self.dtype,
                            shape=[1, 1, 1, ff_filters],
                            initializer=tf.constant_initializer(0.1),
                            trainable=True,
                    ...
                    setattr(
                        ...
                        tf.get_variable(
                            name='%s_ffdrive_beta_%s_%s' % (self.layer_name, idx, il),
                            dtype=self.dtype,
                            shape=[1, 1, 1, ff_filters],
                            initializer=tf.constant_initializer(0.0),
                            trainable=True,
        ...
        for idx, hgru_id in enumerate(self.hgru_ids):
            ...
            with tf.variable_scope('%s_hgru_weights_%s' % (self.layer_name, layer)):
                setattr(
                    self,
                ...
                if self.alpha and not self.lesion_alpha:
                    ...
                    setattr(
                        self, 'alpha_%s' % layer, tf.constant(0.0, dtype=self.dtype)
                    )
                else:
                    setattr(
                        self, 'alpha_%s' % layer, tf.constant(1.0, dtype=self.dtype)
                    )
                ...
                if self.mu and not self.lesion_mu:
                    ...
                    setattr(self, 'mu_%s' % layer, tf.constant(0.0, dtype=self.dtype))
                else:
                    setattr(self, 'mu_%s' % layer, tf.constant(1.0, dtype=self.dtype))
                ...
                if self.multiplicative_excitation:
                    if self.lesion_kappa:
                        setattr(
                            self, 'kappa_%s' % layer, tf.constant(0.0, dtype=self.dtype)
                        )
                    else:
                        setattr(
                            self,
                    ...
                    if self.lesion_omega:
                        setattr(
                            self, 'omega_%s' % layer, tf.constant(0.0, dtype=self.dtype)
                        )
                    else:
                        setattr(
                            self,
                    ...
                    setattr(
                        self, 'kappa_%s' % layer, tf.constant(1.0, dtype=self.dtype)
                    )
                    setattr(
                        self, 'omega_%s' % layer, tf.constant(1.0, dtype=self.dtype)
                    )
                if self.adaptation:
                    setattr(
                        self,
    ...
    def resize_x_to_y(self, x, y, name, mode='transpose', use_bias=True):
        """Resize activity x to the size of y using interpolation."""
        y_size = y.get_shape().as_list()[1:]
        if mode == 'resize':
            raise NotImplementedError
            return tf.image.resize_images(
                x, y_size[:-1], self.resize_kernel, align_corners=True
            )
        elif mode == 'transpose':
            # if self.n is None:
            #     n = 1
            ...
            resized = tf.image.resize_images(
                images=x, size=y_size[:-1], align_corners=True
            )
            resized = tf.nn.conv2d(
                resized, resize_kernel, self.strides, padding=self.padding
            )
            resized = tf.nn.bias_add(resized, resize_bias)
            resized = self.ff_nl(resized)
            return resized
        else:
            raise NotImplementedError(mode)

    def conv_2d_op(self, data, weights, symmetric_weights=False, dilations=None):
    ...
    def circuit_input(self, h2, layer, var_scope, layer_idx):
        ...
        gain_kernels = getattr(self, '%s_gain_kernels_%s' % (self.symm_g_tag, layer))
        ...
        with tf.variable_scope('%s/g1_bn' % var_scope, reuse=self.scope_reuse) as scope:
    ...
    def circuit_output(self, h1, layer, var_scope, layer_idx):
        ...
        mix_kernels = getattr(self, '%s_mix_kernels_%s' % (self.symm_g_tag, layer))
        ...
        with tf.variable_scope('%s/g2_bn' % var_scope, reuse=self.scope_reuse) as scope:
    ...
    def hgru_ops(self, i0, x, h2, layer):
        ...
        c1, g1 = self.circuit_input(
            h2=h2, layer=layer, var_scope=var_scope, layer_idx=layer_idx
        )
        with tf.variable_scope('%s/c1_bn' % var_scope, reuse=self.scope_reuse) as scope:
        ...
        h1 = self.input_integration(x=x, c1=c1, h2=h2, layer=layer)

        # Circuit output receives recurrent input h1
        c2, g2 = self.circuit_output(
            h1=h1, layer=layer, var_scope=var_scope, layer_idx=layer_idx
        )

        with tf.variable_scope('%s/c2_bn' % var_scope, reuse=self.scope_reuse) as scope:
        ...
        h2 = self.output_integration(h1=h1, c2=c2, g2=g2, h2=h2, layer=layer)
    ...
    def upsample_router(self, activity, conv_list, i0):
        ...
        #     return activity
        return self.upsample_ops(activity=activity, conv_list=conv_list, i0=i0)
    ...
    def upsample_ops(self, activity, conv_list, i0):
        ...
        for idx, target in reversed(list(enumerate(conv_list))):
            ...
            if self.batch_norm:
                ...
                with tf.variable_scope(up_scope, reuse=self.scope_reuse) as scope:
                    ...
                    activity = activity * getattr(
                        self, 'resize_gamma_%s' % idx
                    ) + getattr(self, 'resize_beta_%s' % idx)
    ...
    def td_router(self, activity, l1_h2, i0):
        ...
        return self.td_ops(activity=activity, l1_h2=l1_h2, i0=i0)

    def td_ops(self, activity, l1_h2, i0):
        """Apply top-down operations."""
        fb_inh_1, fb_act_1 = self.hgru_ops(i0=i0, x=l1_h2, h2=activity, layer='fb1')
        ...
        for idx, (filters, reps) in enumerate(
            ...
            for il in range(reps):
                activity = tf.nn.conv2d(
                    input=activity,
                    filter=getattr(self, 'intermediate_kernel_%s_%s' % (idx, il)),
                    strides=self.strides,
                    padding=self.padding,
                )
                activity = tf.nn.bias_add(
                    activity, getattr(self, 'intermediate_bias_%s_%s' % (idx, il))
                )
                ...
                if 1:  # idx == (  # Use with the resid cond below
                    ...
                    if self.batch_norm:
                        with tf.variable_scope(
                            ff_scope, reuse=self.scope_reuse
                        ) as scope:
                            activity = tf.contrib.layers.batch_norm(
                                inputs=activity,
                                scale=False,
                            ...
                            activity = activity * getattr(
                                self, 'intermediate_gamma_%s_%s' % (idx, il)
                            ) + getattr(self, 'intermediate_beta_%s_%s' % (idx, il))
    ...
    def full(self, i0, x, l1_h2, l2_h2, fb_act_1):
        ...
        if self.batch_norm:
            with tf.variable_scope(l1_h2_scope, reuse=self.scope_reuse) as scope:
        ...
        activity, conv_list = self.conv_tower(
            activity=l1_h2_pool, pre_pool=l1_h2, i0=i0
        )

        # # LAYER 2 hGRU
        # hGRU
        l2_h1, l2_h2 = self.hgru_ops(i0=i0, x=activity, h2=l2_h2, layer='h2')
        l2_h2_scope = 'l2_h2_bn'
        if not self.while_loop:
            l2_h2_scope = '%s_t%s' % (l2_h2_scope, i0)
        if self.batch_norm:
            with tf.variable_scope(l2_h2_scope, reuse=self.scope_reuse) as scope:
                activity = tf.contrib.layers.batch_norm(
                    inputs=l2_h2,
                    scale=False,
                    center=False,
        ...
        # Add Upsamples
        activity = self.upsample_router(activity=activity, conv_list=conv_list, i0=i0)

        # # LAYER 1 tdGRU
        # Feedback from hgru-2 to hgru-1
        l1_h2, fb_act_1 = self.td_router(activity=activity, l1_h2=l1_h2, i0=i0)
        ...
        if self.while_loop:
            ...
            for idx in range(self.timesteps):
                i0, x, l1_h2, l2_h2, fb_act_1 = self.full(
                    i0=i0, x=x, l1_h2=l1_h2, l2_h2=l2_h2, fb_act_1=fb_act_1
                )
        if self.readout == 'fb':
            return fb_act_1
        else:
