...
def max_pool(
    bottom, name, k=[1, 2, 2, 1], s=[1, 2, 2, 1], p='SAME', data_format='NHWC'
):
    """Local max pooling."""
    return tf.nn.max_pool(
        bottom, ksize=k, strides=s, padding=p, data_format=data_format, name=name
    )
...
def global_pool(bottom, name, aux, data_format='NHWC'):
    ...
    if pool_type == 'average' or pool_type == 'mean':
        activity = tf.reduce_mean(bottom, reduction_indices=reduction_indices)
    elif pool_type == 'max':
        activity = tf.reduce_max(bottom, reduction_indices=reduction_indices)
    else:
        raise NotImplementedError('Cannot understand %s' % pool_type)
    return activity


def unpool_with_argmax_layer(bottom, ind, name, filter_size):
    ...
    if np.any(input_shape != target_shape):
        bottom = tf.image.resize_image_with_crop_or_pad(
            bottom, target_shape[1], target_shape[2]
        )
        input_shape = bottom.get_shape().as_list()
